...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (adId) {
      ...
      if (sessions[adId]) {
        sessions[adId].lastValue = value;
        sessions[adId].lastValueType = valueType;
        sessions[adId].updatedAt = Date.now();
        writeSessions(sessions);
      }
    }

    forwardToBackend(`${WEBHOOK_BASE}/api/sendValue`, body).catch(() => {});

    res.status(200).json({ ok: true, status: 'value_received' });
  } catch (e) {
    res.status(200).json({ ok: true, status: 'value_received' });
  }
}
